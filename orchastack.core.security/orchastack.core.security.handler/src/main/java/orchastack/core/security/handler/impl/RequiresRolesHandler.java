 ...
 * (!subject.hasRole("administrator")) { throw new AuthorizationException(...);
 * }
 */

@Handler(name = RequiresRolesHandler.NAME, namespace = AuthzAnnotationType.NAMESPACE)
public class RequiresRolesHandler extends PrimitiveHandler {
	final private static Logger log = Logger
			.getLogger(RequiresRolesHandler.class.getName());

	/**
	 * The handler name.
	 */
	public static final String NAME = "requiresroles";
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (method == null) {
				throw new ConfigurationException(
						"A RequiresRoles element must specified the method attribute");
			}
			MethodMetadata ma = this.getPojoMetadata().getMethod(method);
			if (ma == null) {
				throw new ConfigurationException(
						"A RequiresRoles method is not in the pojo class : "
								+ method);
			}
			...
			if (value == null) {
				throw new ConfigurationException(
						"A RequiresRoles element must specified the value attribute");
			}

			String logical = elements[i].getAttribute("logical");

			SecuredMethod sm = new SecuredMethod(method,
					AuthzAnnotationType.RequiresRoles, value, logical);
