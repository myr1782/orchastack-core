 ...
 * principals.isEmpty()) { //no identity - they're anonymous, not allowed: throw
 * new AuthorizationException(...); }
 * 
 * @author active
 */
@Handler(name = RequiresUserHandler.NAME, namespace = AuthzAnnotationType.NAMESPACE)
public class RequiresUserHandler extends PrimitiveHandler {
	 ...
	 * The handler name.
	 */
	public static final String NAME = "requiresuser";
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (method == null) {
				throw new ConfigurationException(
						"A RequiresUser element must specified the method attribute");
			}
			MethodMetadata ma = this.getPojoMetadata().getMethod(method);
			if (ma == null) {
				throw new ConfigurationException(
						"A RequiresUser method is not in the pojo class : "
								+ method);
			}

			SecuredMethod sm = new SecuredMethod(method,
					AuthzAnnotationType.RequiresUser, null, null);
