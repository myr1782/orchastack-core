...
public class PersistenceBundleHelper {
	...
	private static final Logger _logger = Logger
			.getLogger(PersistenceBundleHelper.class.getName());
	...
	/**
	 * This method locates persistence descriptor files based on a combination
	 * of the default location "META-INF/persistence.xml" and the
	 * Meta-Persistence header.
	 * 
	 * Note that getEntry is used to ensure we do not alter the state of the
	 * bundle Note also that web application bundles will never return
	 ...
	 * @return
	 */
	public static Collection<PersistenceDescriptor> findPersistenceXmlFiles(
			Bundle bundle) {
		...
		if (metaPersistence == null && exportEJB == null) {
			if (webContextPath == null) {
				...
				// WABs behave a bit differently to normal bundles. We process
				// them even if they don't have a Meta-Persistence

				String bundleClassPath = headers
						.get(Constants.BUNDLE_CLASSPATH);

				locations = findPersistenceXmlOnClassPath(bundleClassPath);

				_logger.log(Level.INFO, "wab.without.meta.persistence.header "
						+ bundleIdentity + PERSISTENCE_UNIT_HEADER + locations);
			}
		} else {

			// Always search the default location, and use a set so we don't
			// search the same
			// location twice!
			...
			if (exportEJB != null && metaPersistence == null) {
				_logger.log(Level.INFO, "ejb.without.meta.persistence.header "
						+ bundleIdentity + PERSISTENCE_UNIT_HEADER + locations);
		...
		for (String location : locations) {
			try {
				...
				if (file != null) {
					persistenceXmlFiles.add(new PersistenceDescriptor(location,
							file));
				}
			} catch (Exception e) {
				_logger.log(Level.SEVERE,
						"exception.while.locating.descriptor " + location, e);
				// If we get an exception, then go through closing all of our
				// streams.
		...
		if (persistenceXmlFiles.isEmpty() && metaPersistence != null) {
			_logger.log(Level.WARNING, "no.persistence.descriptor "
					+ bundleIdentity);
		}
	...
	private static Collection<String> findPersistenceXmlOnClassPath(
