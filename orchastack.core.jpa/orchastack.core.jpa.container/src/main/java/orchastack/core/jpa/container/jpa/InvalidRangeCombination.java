...
/**
 * This exception is thrown when a persistence descriptor contains an set of
 * version ranges with no common overlap
 */
public class InvalidRangeCombination extends Exception {
	...
	public InvalidRangeCombination(Version minVersion, boolean minExclusive,
		...
		super("no.overlap.for.version.range "
				+ getVersionRangeString(minVersion, minExclusive, maxVersion,
						maxExclusive));
	}
	...
	private static String getVersionRangeString(Version minVersion,
		...
		if (maxVersion == null)
			...
			return ((minExclusive) ? "(" : "[") + minVersion + "," + maxVersion
					+ ((maxExclusive) ? ")" : "]");
	}
}
