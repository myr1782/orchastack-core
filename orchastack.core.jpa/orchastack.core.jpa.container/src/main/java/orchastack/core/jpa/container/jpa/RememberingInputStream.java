...
/**
 * This class can be used to buffer an arbitrary amount of content from an input
 * stream and be able to reset to the start.
 */
public class RememberingInputStream extends InputStream {
	 ...
	 * 
	 * @param i
	 *            one less than the number of bytes that need to be held.
	 */
	private void ensureCapacity(int i) {
		if ((pos + i) >= bytes.length) {
			...
			System.arraycopy(tmp, 0, bytes, 0, (maxRead >= pos) ? maxRead + 1
					: pos);
		}
	}
	...
	public int read(byte[] b, int off, int len) throws IOException {
		if (pos <= maxRead) {
			if (pos + len <= maxRead) {
				...
				int read = stream.read(b, off + lengthLeftOfBuffer, len
						- lengthLeftOfBuffer);
				if (read < 0) {
				...
				System.arraycopy(b, off + lengthLeftOfBuffer, bytes,
						maxRead + 1, read);
	...
	public long skip(long n) throws IOException {
		throw new IOException("This input stream does not support skip.");
	}
	...
	/**
	 * Noop. Does not close the passed in archive, which is kept open for
	 * further reading.
	 */
	@Override
	...
	/**
	 * Actually closes the underlying InputStream. Call this method instead of
	 * close, which is implemented as a no-op. Alternatively call close directly
	 * on the parent.
	 * 
	 * @throws IOException
	 */
