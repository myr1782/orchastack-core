 ...
 * transactions. Important consideration such as connection pooling and error
 * handling are completely ignored.
 * 
 */
public class XADatasourceEnlistingWrapper implements DataSource, Serializable {
	...
	public Connection getConnection() throws SQLException {
		...
		if (transaction != null) {
			...
			return getEnlistedConnection(wrappedDS.getXAConnection()
					.getConnection(), false);
		}
	}
	...
	public Connection getConnection(String username, String password)
		...
		if (transaction != null) {
			...
			Connection connection = connectionMap.get(key);
			if (connection == null) {
				XAConnection xaConnection = wrappedDS.getXAConnection(username,
						password);
				connection = xaConnection.getConnection();
				enlist(transaction, xaConnection.getXAResource(), key);
				connectionMap.put(key, connection);
			...
			return getEnlistedConnection(
					wrappedDS.getXAConnection(username, password)
							.getConnection(), false);
		}
	}
	...
	private Transaction getTransaction() throws SQLException {
		try {
			return (OSGiTransactionManager.get().getStatus() == Status.STATUS_ACTIVE) ? OSGiTransactionManager.get().getTransaction() : null;
		} catch (SystemException e) {
			throw new SQLException("unable.to.get.tx", e);
		}
	}
	...
	private Connection getEnlistedConnection(Connection connection,
	...
	public boolean equals(Object other) {
		...
		if (other.getClass() == this.getClass()) {
			return wrappedDS
					.equals(((XADatasourceEnlistingWrapper) other).wrappedDS);
		}
