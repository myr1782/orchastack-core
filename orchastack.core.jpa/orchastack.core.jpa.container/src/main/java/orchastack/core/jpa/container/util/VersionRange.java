...
public final class VersionRange {
	...
	private static final Pattern versionCapture = Pattern
			.compile("\"?(.*?)\"?$");
	 ...
	 * @param exactVersion
	 *            whether this is an exact version {@code true} or goes to
	 *            infinity {@code false}
	 */
	public VersionRange(String version, boolean exactVersion) {

		if (exactVersion) {
			// Do not store this string as it might be just a version, or a
			// range!
	 ...
	 */
	private VersionRange(Version maximumVersion, boolean maximumExclusive,
	...
	/**
	 * this method returns the exact version from the versionInfo obj. this is
	 * used for DeploymentContent only to return a valid exact version
	 ...
	 * @throws IllegalArgumentException
	 */
	private boolean processExactVersionAttribute(String version)
			throws IllegalArgumentException {
		...
		if (!minimumVersion.equals(maximumVersion)) {
			throw new IllegalArgumentException("UTIL0011E: Unable to parse "
					+ version + " for the exact version.");
		}

		if (!!!isExactVersion()) {
			throw new IllegalArgumentException(
					"UTIL0009E: Unable to parse the version " + version);
		}
	 ...
	 * @throws IllegalArgumentException
	 */
	private boolean processVersionAttribute(String version)
			throws IllegalArgumentException {
		boolean success = false;

		if (version == null) {
			throw new IllegalArgumentException(
					"UTIL0010E: Unable to parse the version. Could not parse empty string.");
		}
		...
		if (matches.matches()) {
			...
			if ((versions.startsWith("[") || versions.startsWith("("))
				...
				String maxVersion = versions.substring(index + 1,
						versions.length() - 1);
				...
				try {
					...
					throw new IllegalArgumentException(
							"UTIL0009E: Unable to parse the version. Could not parse "
									+ nfe);
				}
			} else {
				try {
	...
	private void assertInvariants() {
		if (minimumVersion == null
				|| !isRangeValid(minimumVersion, minimumExclusive,
						maximumVersion, maximumExclusive)) {
	 ...
	 */
	private boolean isRangeValid(Version min, boolean minExclusive,
			Version max, boolean maxExclusive) {
		boolean result;
	...
	/**
	 * Create a new version range that is the intersection of {@code this} and
	 * the argument. In other words, the largest version range that lies within
	 * both {@code this} and the parameter.
	 * 
	 * @param r
	 *            a version range to be intersected with {@code this}.
	 * @return a new version range, or {@code null} if no intersection is
	 *         possible.
	 */
	public VersionRange intersect(VersionRange r) {
		...
		if (maximumVersion == null) {
			...
			if (maxCompare < 0) {
				...
				newMaximumVersion = maximumVersion;
				newMaximumExclusive = (maximumExclusive || r
						.isMaximumExclusive());
			}
		}

		VersionRange result;
		if (isRangeValid(newMinimumVersion, newMinimumExclusive,
				newMaximumVersion, newMaximumExclusive)) {
			result = new VersionRange(newMaximumVersion, newMaximumExclusive,
