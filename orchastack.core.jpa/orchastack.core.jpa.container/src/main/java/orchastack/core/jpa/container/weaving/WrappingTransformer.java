 ...
 * under the License.
 */
package orchastack.core.jpa.container.weaving;
...
class WrappingTransformer implements ClassTransformer {
	...
	public WrappingTransformer(ClassTransformer delegate,
		...
		if (delegate == null)
			throw new NullPointerException("jpa.weaving.null.transformer");

		if (persistenceProvider == null) {
			throw new NullPointerException("jpa.weaving.null.provider");
		}

		this.delegate = delegate;

		Object packages = persistenceProvider
				.getProperty("jpa.container.weaving.packages");
		...
		if (packages instanceof String[]) {
			...
			String suffix = ";" + Constants.BUNDLE_SYMBOLICNAME_ATTRIBUTE + "="
					...
					+ provider.getVersion();
			BundleWiring pbw = (BundleWiring) provider
					.adapt(BundleWiring.class);
			BundleRevision br = pbw.getRevision();
			for (Object bco : br
					.getDeclaredCapabilities(BundleRevision.PACKAGE_NAMESPACE)) {
				BundleCapability bc = (BundleCapability) bco;
				packageImportsToAdd.add(bc.getAttributes().get(
	...
	public String toString() {
		return "Transformer: " + delegate.toString() + " Packages to add: "
				+ packageImportsToAdd;
	}
}