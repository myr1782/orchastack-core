...
/**
 * This is a simple temporary ClassLoader that delegates to the Bundle, but does
 * not call loadClass. It is used by the PersistenceUnitInfo
 */
public class TempBundleDelegatingClassLoader extends ClassLoader {
	...
	private void updateContext(Bundle currentContext, String className) {
		...
		BundleWiring wiring = (BundleWiring) currentContext
				.adapt(BundleWiring.class);
		for (Object wo : wiring
				.getRequiredWires(BundleRevision.PACKAGE_NAMESPACE)) {
			BundleWire wire = (BundleWire) wo;
			if (wire.getCapability().getAttributes()
	...
	protected URL findResourceInBundle(final String resName,
			final Bundle inBundle) {
		// Bundle.getResource requires privileges that the client may not have
		// but we need
	...
	private InputStream getResourceInBundleAsStream(final String resName,
			final Bundle inBundle) {
	...
	protected Enumeration<URL> findResourcesInBundle(final String resName,
			final Bundle inBundle) throws IOException {
		Enumeration<URL> resources = null;
		try {
			// Bundle.getResources requires privileges that the client may not
			// have but we need
			// use a doPriv so that only this bundle needs the privileges
			resources = AccessController
			...
			// thrownException can never be a RuntimeException, as that would
			// escape
			// the doPriv normally
